"""
OpenAI Agents SDK integration with MCP servers - Multi-Agent Orchestration with Planning.
This variant implements a Manager (agents as tools) pattern:
- Manager agent: Orchestrates the planning and execution process
- Planner agent: Exposed as a tool, generates 5-10 high-level query strategies
- Execution agent: Exposed as a tool, executes strategies using MCP tools
...
router = APIRouter(
    prefix="/openai-agents-with-planning", tags=["openai-agents-with-planning"]
)
...
# Set OPENAI_AGENTS_WITH_PLANNING_MODEL env var to override
PLANNING_MODEL = os.getenv("OPENAI_AGENTS_WITH_PLANNING_MODEL", "gpt-4o")
...
class QueryRequest(BaseModel):
    """Request model for planning agent queries."""
...
def create_execution_agent(model: str, server) -> Agent:
    """
    Create the execution agent that executes query strategies.
...
def create_manager_agent(
    planner_agent: Agent, execution_agent: Agent, model: str
) -> Agent:
    """
    Create the manager agent that orchestrates planning and execution.
    ...
    return Agent(
        ...
        tools=[
            ...
            execution_agent.as_tool(
                ...
                tool_description="Executes a specific query strategy using available tools: MCP tools (get_schema, run_query) and custom tools (vector_search_on_chunks).",
            ),
        ],
        model_settings=ModelSettings(
...
@router.post("/query")
async def query_with_planning(request: QueryRequest):
    """
    Query the knowledge graph using multi-agent orchestration with Manager pattern.

    This variant implements the Manager (agents as tools) pattern:
    ...
    - Execution agent is exposed as a tool that executes strategies using MCP tools
    ...
    try:
        ...
        async with MCPServerStreamableHttp(
            ...
            planner_agent = create_planner_agent(PLANNING_MODEL)
            execution_agent = create_execution_agent(PLANNING_MODEL, intercepting_server)

            # Create manager agent that orchestrates the sub-agents as tools
            manager_agent = create_manager_agent(
                planner_agent, execution_agent, PLANNING_MODEL
            )

            logger.info(f"Running manager agent with session {session.session_id}...")
        ...
        logger.error(f"Error running planning agent: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Error running planning agent: {str(e)}"
        )
